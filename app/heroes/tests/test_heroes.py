...
@pytest.mark.asyncio
async def test_create_hero(async_client: AsyncClient, async_session: AsyncSession, test_data: dict):
...
@pytest.mark.asyncio
async def test_get_hero(async_client: AsyncClient, async_session: AsyncSession, test_data: dict):
...
@pytest.mark.asyncio
async def test_patch_hero(async_client: AsyncClient, async_session: AsyncSession, test_data: dict):
    hero_data = test_data["initial_data"]["hero"]
    statement = insert(Hero).values(hero_data)
    await async_session.execute(statement=statement)
    await async_session.commit()
...
@pytest.mark.asyncio
async def test_delete_hero(async_client: AsyncClient, async_session: AsyncSession, test_data: dict):
    hero_data = test_data["initial_data"]["hero"]
    statement = insert(Hero).values(hero_data)
    await async_session.execute(statement=statement)
    await async_session.commit()
